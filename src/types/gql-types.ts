...
export interface OrgReposQueryVariables { login: string, cursor?: string|null, }
;
...
export interface IssuesQueryVariables {
  owner: string, name: string, cursor?: string|null,
}
;
...
export interface RepoCommitsQueryVariables {
  owner: string, name: string, cursor?: string|null, since?: string|null,
}
;
...
export interface RepoCommitsQuery {
  ...
  repository: {
    ...
    defaultBranchRef: {
      ...
      target: ({
        __typename: 'Commit',
        // The linear commit history starting from (and including) this commit,
        // in the same order as `git log`.
        history: {
          __typename: string,
          // Information to aid in pagination.
          pageInfo: {
...
export interface PullRequestCommitsQuery {
  ...
  node: ({
    ...
    __typename: 'PullRequest',
    // A list of commits present in this pull request's head branch not present
    // in the base branch.
    commits: {
      __typename: string,
      // Information to aid in pagination.
...
export interface ViewerPullRequestsQueryVariables {
  login: string, reviewRequestsQueryString: string, reviewedQueryString: string,
}
;
  ...
  user: {
    ...
    pullRequests: {
      ...
      rateLimit: {
        ...
        cost: number,
        // The maximum number of points the client is permitted to consume in a
        // 60 minute window.
        limit: number,
...
export interface MyReposQueryVariables { login: string, cursor?: string|null, }
;
