...
export async function fetchOutgoingData(
  ...
  if (viewerPrsResult.data.user) {
    ...
    const outgoingPrPromises =
      ...
      const prs = viewerPrsResult.data.user.pullRequests;

      // Set pagination info.
      totalCount = prs.totalCount;
      hasMore = prs.pageInfo.hasPreviousPage;
      cursor = prs.pageInfo.startCursor;
  ...
  export async function fetchIncomingData(
      dashboardLogin: string,
      token: string): Promise<api.IncomingDashResponse> {
    const openPrQuery = 'is:open is:pr archived:false';
    const reviewedQueryString = `reviewed-by:${dashboardLogin} ${
        openPrQuery} -author:${dashboardLogin}`;
    const reviewRequestsQueryString =
        `review-requested:${dashboardLogin} ${openPrQuery}`;
    const mentionsQueryString =
        `${reviewedQueryString} mentions:${dashboardLogin}`;
  ...
  function getLastMentioned(
      pullRequest: mentionedFieldsFragment, login: string): api.MentionedEvent|
      null {
    let latest = null;
